  ...
  const handleFileUpload = async (index: number, file: File) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Файлды жүктей алмады: ${errorMessage}`);
        console.error('Upload error:', errorData);
      }
    } catch (error) {
      alert(`Файлды жүктеу қатесі: ${error instanceof Error ? error.message : 'Белгісіз қате'}`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900">Мақаланы өңдеу</h1>
          <Link
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <label className="flex items-center gap-2">
            <input
              ...
              className="rounded"
            />
            <span className="text-sm font-medium text-gray-700">Жарияланған</span>
          </label>
        </div>
